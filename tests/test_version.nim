...
suite "one version, six copies":
  let manifest = valueOf("UniDatabase.nimble", "version", "\"", "\"")
  ...
  test "the Nim constant agrees":
    check UniDatabaseVersion == manifest

  test "the C header agrees, macros and string alike":
    let parts = manifest.split('.')
    check valueOf("include/UniDatabase.h", "UNIDATABASE_VERSION_MAJOR", " ",
        "") == parts[0]
    check valueOf("include/UniDatabase.h", "UNIDATABASE_VERSION_MINOR", " ",
        "") == parts[1]
    check valueOf("include/UniDatabase.h", "UNIDATABASE_VERSION_PATCH", " ",
        "") == parts[2]
    check valueOf("include/UniDatabase.h", "define UNIDATABASE_VERSION ", "\"",
        "\"") == manifest

  test "the C ABI reports it":
    # Read from the source rather than called: this suite links no C library.
    check valueOf("src/UniDatabase/c_api.nim", "UniDatabaseVersionC", "\"",
        "\"") == manifest
  ...
  test "the Python test expects it":
    check valueOf("py/tests/test_fibonacci.py", "unidatabase.version()", "\"",
        "\"") == manifest
...
suite "one domain bound, two copies":
  ...
  test "the C macro agrees with the Nim constant":
    check valueOf("include/UniDatabase.h", "UNIDATABASE_FIB_MAX_N", " ", "") == $FibMaxN
