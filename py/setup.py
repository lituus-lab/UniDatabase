...
"""Build unidatabase._core, a Cython extension over the UniDatabase C ABI.
...
PKG_DIR = os.path.join(HERE, "unidatabase")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniDatabase.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniDatabase.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniDatabase.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniDatabase.so", True
...
def nim_project_dir():
    """Where UniDatabase.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unidatabase from "
...
# The sdist ships the pre-transpiled unidatabase/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unidatabase", "_core.pyx")
ext = Extension(
    "unidatabase._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unidatabase", "_core.c")],
    include_dirs=[INCLUDE],
    library_dirs=[LIB_DIR],
    extra_link_args=LINK_ARGS,
    libraries=["UniDatabase"],
)
...
setup(
    ...
    package_data={"unidatabase": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unidatabase": ["_core.c"]},
    zip_safe=False,
)
