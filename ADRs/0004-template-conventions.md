...
- Status: Accepted
- Date: 2026-07-15
- Scope: UniDatabase and the conventions every engine started from it inherits
...
src/UniDatabase/fibonacci.nim  hello-world (NimContracts)
...
- C symbol prefix: the library's own name in lower case (`unidatabase_`, so
...
| `UniDatabase` | `UniFoo` |
| `unidatabase` | `unifoo` |
| `libUniDatabase` | `libUniFoo` |
| `UniDatabase.h` | `UniFoo.h` |
| `lituus-unidatabase` | `lituus-unifoo` |
