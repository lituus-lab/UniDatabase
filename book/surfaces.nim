...
`unidatabase_fibonacci(n)` answers for every `int` a caller can pass. Below the
...
nbCode:
  echo run("cc -Iinclude -o build/book_c_demo book/surfaces_demo.c " &
          "libUniDatabase.a 2>&1 && ./build/book_c_demo")
...
- **`_core` is importable and unchecked.** `unidatabase._core.fibonacci` is the
