...
```bash
nimble install https://github.com/lituus-lab/UniDatabase    # Nim
pip install lituus-unidatabase                              # Python
```

For C, the build produces `libUniDatabase.a` and `include/UniDatabase.h`:

```bash
build/unigate clibStatic
cc -Iinclude your.c libUniDatabase.a
```

The PyPI distribution is `lituus-unidatabase`; the import name stays
`unidatabase`. Those are two decisions, and the bare names are not all
available.
...
nbCode:
  ...
  echo "version ", UniDatabaseVersion
