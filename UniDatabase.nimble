...
const
  sharedLib =
    when defined(windows): "libUniDatabase.dll"
    elif defined(macosx): "libUniDatabase.dylib"
    else: "libUniDatabase.so"
  staticLib = "libUniDatabase.a"  # MinGW `ar` on Windows, so `.a` everywhere.
