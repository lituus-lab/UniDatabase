...
src/UniDatabase/fibonacci.nim  Nim core (NimContracts)
...
|---|---|---|
| `UniDatabase` | `UniFoo` | `UniAccurate`, `UniMath` |
| `unidatabase` | `unifoo` | `uniaccurate`, `unimath` |
| `libUniDatabase` | `libUniFoo` | `libUniAccurate` |
| `UniDatabase.h` | `UniFoo.h` | `UniAccurate.h` |
| `lituus-unidatabase` | `lituus-unifoo` | `lituus-uniaccurate` |

The C symbol prefix is the library's own name in lower case —
`unidatabase_fibonacci`, so `unifoo_*`. Short prefixes read better and collide:
a binary that links several engines at once holds them all in one namespace.

Files to rename: `UniDatabase.nimble`, `src/UniDatabase.nim`, `src/UniDatabase/`,
`include/UniDatabase.h`, `tests/c/test_unidatabase.c` (+ its Makefile target),
`py/unidatabase/`. Then update `LICENSE`/`NOTICE` copyright and the ADR titles.
